import "./datatableRoomNumbers.scss";
...
const DatatableRoomNumbers = ({columns}) => {
  const location = useLocation();
  const [list, setList] = useState();
  const path = capitalizeWord(location.pathname.split("/")[1]);
  const id = location.pathname.split("/")[3];
  const { data, loading, error } = useFetch(`/Room/GetRoomNumbersByRoomId/${id}`);
  ...
  const handleDelete = async (id) => {
    try{
      ...
      await axios.delete(`/RoomNumber/${id}`);
...
export default DatatableRoomNumbers;
